...
function candleInfoContent(point){{if(!point)return {{title:'选中K线',html:'<div class="snapshot-grid"><span>时间</span><b>--</b></div>'}};const o=candleOhlc(point);if(!o)return {{title:'选中K线',html:'<div class="snapshot-grid"><span>数据</span><b>--</b></div>'}};const bar=point.bar||monitorBar,up=o.close>=o.open,color=up?'#fb7185':'#22c55e',vol=point.volume!=null&&point.volume!==''?fmt(point.volume,2):'--',confirmed=String(point.confirmed)==='1'?'已确认':(String(point.confirmed)==='0'?'未确认':'--'),amp=o.low?((o.high-o.low)/o.low*100):0;return {{title:'选中K线 · '+bar,html:'<div class="snapshot-grid"><span>时间</span><b>'+compactTime(point.time)+'</b><span>开盘</span><b>'+fmt(o.open,2)+'</b><span>最高</span><b>'+fmt(o.high,2)+'</b><span>最低</span><b>'+fmt(o.low,2)+'</b><span>收盘</span><b style="color:'+color+'">'+fmt(o.close,2)+'</b><span>涨跌</span><b style="color:'+color+'">'+candleChangeText(o.open,o.close)+'</b><span>振幅</span><b>'+fmt(amp,2)+'%</b><span>成交量</span><b>'+vol+'</b><span>状态</span><b>'+confirmed+'</b></div>'}};}}
